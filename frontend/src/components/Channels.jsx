...
const renderRemovableChannel = (channel, activeChannel, setActiveChannel, setactiveChannelName, setModalInfo) => {
  ...
  return (
    ...
    <Dropdown className={buttonClasses}>
      <Button variant={buttonVariant} onClick={() => handleActiveChannel(channel, setActiveChannel, setactiveChannelName)}># {channel.name}</Button>
...
const renderChannels = (channels, activeChannel, setActiveChannel, setactiveChannelName, setModalInfo) => {
  const channelsHtml = channels.map((channel) => {
    const buttonClasses = `w-100 rounded-0 text-start btn ${channel.id === activeChannel ? 'btn-secondary' : ''}`;
    const buttonVariant = `${channel.id === activeChannel ? 'secondary' : 'light'}`;
    if (channel.removable) return renderRemovableChannel(channel, activeChannel, setActiveChannel, setactiveChannelName, setModalInfo);
    const channelHtml =
      <li key={channel.id}>
...
const Channels = ({ channels, activeChannel, setActiveChannel, setactiveChannelName, setModalInfo }) => {
  ...
  return (
    <div className='col-4 col-md-2 border-end px-0 bg-light flex-column h-100 d-flex'>
      ...
      >
        {renderChannels(channels, activeChannel, setActiveChannel, setactiveChannelName, setModalInfo)}
      </ul>
  </div>
  )
};
